...
st.title("📊 Government Budget Expenditure Predictor")

# --- 2. Load Data and Train Model ---
# We use @st.cache_data so the model only trains once when the app starts, making it fast.
@st.cache_data
def load_and_train():
    ...
    base_tree = DecisionTreeRegressor(max_depth=6, min_samples_split=5, min_samples_leaf=3, random_state=42)
...
try:
    df, model, le, country_mapping, TARGET_COLS = load_and_train()
...
if st.sidebar.button("Predict Expenditure"):
    ...
    ax.bar(x + width/2, results_df["Predicted (%)"], width=width, label='Predicted Data', color='skyblue')
